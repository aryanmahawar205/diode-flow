"""
Receiver streaming pipeline (REWRITTEN STABLE VERSION)

KEY DESIGN CHANGE:
- K_prime is NEVER recomputed from math
- It is ALWAYS derived from actual packets in pool
- One source of truth = EncodedPacket.source_chunk_count

This removes ALL LT graph inconsistency bugs.
"""
...
from common.models import TransferManifest, TransferProgress, EncodedPacket
...
def run_receiver(
    ...
    m_stats = {
        "fountain_recovered_by_window": {},
        "rs_recovered_by_window": {},
        "failed_chunks_by_window": {},
        "last_decode_attempt_time": {},
        "last_attempted_pool_size": {}
    }
        ...
        while True:
            ...
            if raw is None:
                ...
                if manifest and time.time() - last_packet > 10:
                    _check_decode_ready(
                        manifest, pooler, fdec,
                        window_files, window_padding,
                        progress, m_stats, executor, state_lock
                    )

                if manifest and len(window_files) == manifest.total_windows:
                    return _finish(manifest, window_files, storage_dir, progress, m_stats, t_start)
            ...
            if manifest is None:
                ...
                if m and validate_manifest(m)[0]:
                    manifest = m
                    progress = TransferProgress(m.transfer_id, m.file_name, m.total_windows)
            ...
            ok, _ = validate_packet_dict(pkt_dict, manifest, t_start)
            if not ok:
                continue
            ...
            pooler.add(manifest.transfer_id, wid, pkt)

            window_padding[wid] = pkt_dict["padding_length"]

            if progress:
                progress.total_packets_rx += 1

            # ----------------------------------------------------
            # DERIVE K_prime FROM ACTUAL POOL (CRITICAL FIX)
            # ----------------------------------------------------
            pool = pooler.get_pool(manifest.transfer_id, wid)
            if not pool:
                continue

            K_prime = pool[0].source_chunk_count
            ...
            if pooler.is_ready(manifest.transfer_id, wid, K_prime):
                ...
                last_t = m_stats["last_decode_attempt_time"].get(wid, 0)
                last_s = m_stats["last_attempted_pool_size"].get(wid, -1)

                if size > last_s and (last_t == 0 or now - last_t > 5):

                    m_stats["last_decode_attempt_time"][wid] = now
                    m_stats["last_attempted_pool_size"][wid] = size

                    executor.submit(
                        _decode_and_store,
                        manifest, wid, K_prime,
                        pooler, fdec,
                        window_files, window_padding,
                        progress, m_stats,
                        t_start, state_lock
...
def _decode_and_store(
    ...
    missing_before = sum(1 for c in result.chunks if c is None)

    recovered = rs_recover(result.chunks, manifest, manifest.chunk_size)

    missing_after = sum(1 for c in recovered if c is None)
...
# ------------------------------------------------------------
# READY CHECK (SIMPLIFIED)
# ------------------------------------------------------------

def _check_decode_ready(
    manifest, pooler, fdec,
    window_files, window_padding,
    progress, m_stats,
    executor, state_lock
...
def _finish(manifest, window_files, storage_dir, progress, m_stats, t_start):

    logger.info("Assembling file...")

    compressed = Path(QUARANTINE_DIR) / f"{manifest.transfer_id}_compressed"

    if not assemble(window_files, manifest.total_windows, compressed, manifest.file_sha256):
        return False

    if not verify_file(compressed, manifest):
        return False

    final = Path(QUARANTINE_DIR) / manifest.file_name

    if not decompress(compressed, final,
    ...
    store(final, storage_dir, manifest, stats)
