...
Readiness trigger: pool >= K_prime * 1.30 OR idle timeout.
...
class Pooler:
    ...
    def is_ready(self, transfer_id: str, window_id: int, K_prime: int) -> bool:
        """
        STRICT RULE:
        Only decode when we have enough redundancy OR idle timeout fires.
        """
        if self.count(transfer_id, window_id) >= int(K_prime * 1.30):
            return True
    ...
    def clear_window(self, transfer_id: str, window_id: int) -> None:
        if transfer_id in self._pools and window_id in self._pools[transfer_id]:
            to_remove = {
                k for k in self._dedup[transfer_id]
